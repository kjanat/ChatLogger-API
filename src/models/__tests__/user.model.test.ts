...
describe('User Model', () => {
    ...
    beforeAll(async () => {
        await userModelTest.setupTestDB();
    });

    // Clear database between tests
    beforeEach(async () => {
        await userModelTest.setupTestDB.clearDatabase();
    });

    // Disconnect and close the db after tests
    afterAll(async () => {
        await userModelTest.setupTestDB.closeDatabase();
    });
    ...
    it('should create a valid user', async () => {
        const validUser = {
            ...
            lastName: 'User',
            role: 'user',
            organizationId: new userModelTest.mongoose.Types.ObjectId()
        };
        
        const user = new userModelTest.User(validUser);
        await user.save();
        
        const savedUser = await userModelTest.User.findById(user._id);
    ...
    it('should trim whitespace from username', async () => {
        ...
        const user = new userModelTest.User(userData);
        await user.save();
    ...
    it('should enforce unique username and email', async () => {
        ...
        await new userModelTest.User(userData1).save();
        
        // Try username duplicate
        let duplicateUsernameError: any;
        try {
            await new userModelTest.User({
                ...userData1,
