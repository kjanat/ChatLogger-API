...
interface Config {
    ...
    apiBasePath: string;
    apiVersion: string;
    apiEffectivePath: string;
    documentationSlug: string;
    documentationUrlSlug: string;
    apiDocumentationPath: string;
    apiDocumentationUrl: string;
    resetConfigForTests?: () => Config;
}
...
const getMongoDbUri = (): string => {
    ...
    const envSuffixMap: EnvSuffixMap = {
        production: 'PRD',
        staging: 'STG',
        development: 'DEV',
        testing: 'TST',
    ...
    const mongoUri = envSpecificUriVar && process.env[envSpecificUriVar] 
        ? process.env[envSpecificUriVar] 
...
if (process.env.NODE_ENV === 'test') {
    config.resetConfigForTests = function (): Config {
        this.mongoUri = getMongoDbUri();
        this.jwtSecret = getJwtSecret();
        this.host = 'localhost';
        logger.debug('Config reset for tests:', {
            mongodbUriSet: !!this.mongoUri,
